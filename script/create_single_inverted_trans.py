...
parser.add_argument('n', action='store', type=int, help='Identifier of arrow which will be inverted')
...
main_model_lines = main_model_lines.replace(REG_MODEL_FILENAME, REG_MODEL_FILENAME.replace(".mdl", "_%d.mdl" %N))
main_model_lines = main_model_lines.replace(DIST_MODEL_FILENAME, DIST_MODEL_FILENAME.replace(".mdl", "_%d.mdl" %N))

with open( DIR_NAME+ "main_model_%d.mdl" %N , 'w') as main_output:
    for line in main_model_lines:
        main_output.write(line)

#Copy regular submodel to output dir
shutil.copy ( REG_MODEL_FILEPATH, DIR_NAME + REG_MODEL_FILENAME.replace(".mdl", "_%d.mdl" %N) )
#Copy workspace of disturbed submodel
shutil.copy ( WORKSPACE_FILEPATH, DIR_NAME + WORKSPACE_FILENAME.replace(".m", "_%d.m" %N) )

#Copy the first part of disturbed submodel to output dir
OUT_NAME = DIR_NAME + DIST_MODEL_FILENAME.replace(".mdl", "_%d.mdl" %N)
with open( OUT_NAME , 'w') as output:
    for line in out_model_lines:
        if(WORKSPACE_FILENAME in line):
            line=line.replace(WORKSPACE_FILENAME, WORKSPACE_FILENAME.replace(".m", "_%d.m" %N))
        output.write(line)

#Transition mutations
transition = transitions_par[N]  #Get the desired transition
out_trans_lines = []
src_begin   = 0
src_end     = 0
dst_begin   = 0
...
#The first element of transitions_par is empty, so we have to avoid to create such model because it will be identical to the original model
CHECK_IF_EMPTY      = (N==0)
...
if not(CHECK_IF_INVERTIBLE) or CHECK_IF_EMPTY:
    shutil.rmtree( OUT_DIR + OUT_SUBDIR_NAME + "%d/" %(N))
    result=False
...
print ">output directory:\t%s"  %str( DIR_NAME )
